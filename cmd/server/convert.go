...
func (c *convertReqObj) Validate() error {
	...
	return validation.ValidateStruct(c,
		...
		validation.Field(&c.From, validation.Required, validation.In(mex.CurrencyListArr...)),
		validation.Field(&c.To, validation.Required, validation.In(mex.CurrencyListArr...)),
		validation.Field(&c.Exchanger, validation.Required),
	)

	//if mex.CurrencyList[c.To] == "" || mex.CurrencyList[c.From] == "" {
...
var ConvertGet = func(w http.ResponseWriter, r *http.Request) {
  ...
  "to": "%s",
  "cacheTime":"%s"
}`, amount, exchanger, from, to, cacheTime)
...
var ConvertPost = func(w http.ResponseWriter, r *http.Request) {
	...
	if string(currencyCachedVal) == "" {
		...
		for _, v := range convertReq.Exchanger {
			...
			var e mex.Exchanger
			opt := map[string]string{`userAgent`: v.UserAgent, `apiVersion`: v.ApiVersion}
